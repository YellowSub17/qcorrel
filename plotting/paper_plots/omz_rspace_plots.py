...
for rmax, padf_fname in zip(rmaxs,padf_fnames):
    ...
    t_space = np.linspace(0,tmax, cropt)
    ...
    np.savetxt('4omz_padf_r1r2.dat', r1r2, header=f'{r1r2.shape}')
    ...
    for col, dist in zip(cols,dists):
        plt.plot(t_space, (dist/2)/np.sin(np.radians(t_space)/2),\
                 color=col, label=f'{dist} $\AA$')
