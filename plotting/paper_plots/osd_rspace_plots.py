...
padf_fnames = [
    ...
    '4omz-sf_paper_qcorrel_wonl0_padf',
]
...
for rmax, padf_fname in zip(rmaxs,padf_fnames):
    ...
    r_space = np.linspace(rmin,rmax, ncropr-nrmin)
    ...
    # np.savetxt('1al1_padf_r1r2.dat', r1r2, header=f'{r1r2.shape}')
...
#    r1r2 *= rscale**2
    ...
    ppu.plot_map(r1r2, title=f'{padf_fname[:4].upper()} - PADF',
          extent=[0, cropt,rmin,cropr], cmap='viridis', save=f'{padf_fname[:4]}_r1r2', xlabel='$\\theta$ / $ ^{\circ}$', ylabel='$r_1 = r_2$ / $\\AA$')

    dists = [5,6.9, 9.5]
    cols = ['red',np.array([193/255,77/255,0]), 'yellow']
    for col, dist in zip(cols,dists):
        plt.plot(t_space[90:179], (dist/2)/np.sin(np.radians(180-t_space[90:179])/2),'-.',color=col, label=f'{dist} $\AA$')
